...
Logger = (function() {
  ...
  Logger.prototype.error = function() {
    ...
    if (parms.length > 0) {
      ...
      this._log('error', parms, 'red+bold', 'Error');
      if (this.config && this.config.isBuild && this.config.exitOnError && exitIfBuild) {
  ...
  Logger.prototype.success = function() {
    var options, parms, s, title, _i, _ref;
    parms = 2 <= arguments.length ? __slice.call(arguments, 0, _i = arguments.length - 1) : (_i = 0, []), options = arguments[_i++];
    if (parms.length === 0) {
      parms = [options];
    }
    title = options === true ? "Success" : ((_ref = this.config) != null ? _ref.growl : void 0) != null ? (s = this.config.growl.onSuccess, this.isStartup && !this.config.growl.onStartup ? null : !options || (options.isJavascript && s.javascript) || (options.isCSS && s.css) || (options.isTemplate && s.template) || (options.isCopy && s.copy) ? "Success" : null) : "Success";
